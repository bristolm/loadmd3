...
void Build(LWObjectImport *funcs, UnrealSkeletalModel& model)
{
	...
	for (idx = 0; idx < model.MaterialCount(); idx ++)
	{
		...
		// Try and prevent duplicate names
	...
	for (idx = 0; idx < model.TriangleCount(); idx ++)
	{
		...
		funcs->polTag(funcs->data, polyid,
						LWPTAG_SURF,	// type
						SurfaceNames[tri.MatIndex]);

		// Add in the VMAPs on a per-poly basis
		for (int j = 0; j < 3; j++)
		{
...
int Load_MDL(LWObjectImport *funcs)
{
	HalfLifeModel		model(funcs->filename);
